...
- `/novel-init <title>`：创建项目骨架并强制生成 `.opencode/novel/profile.md`
...
- `/novel-outline`（优先消费 profile，缺失时自动补跑 compact）
- `/novel-character <id>`
- `/novel-faction <id>`
- `/novel-thread <thread_id>`
- `/novel-chapter-plan <chapter_id> [--apply] [--skip-profile]`：默认写 `.plan.md`，不覆盖原章
...
- 命令会优先复用 `.opencode/novel/profile.md`
- 画像不存在时自动执行六维判定 + `profile-aggregator`（compact）
- 仅在显式 `--skip-profile` 时允许跳过，并应记录 `profile=none`
