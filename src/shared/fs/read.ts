...
function stripBom(buf: Uint8Array): { buf: Uint8Array; bom: "utf8" | "utf16le" | "utf16be" | null } {
  if (startsWithBytes(buf, UTF8_BOM)) return { buf: buf.slice(UTF8_BOM.length), bom: "utf8" };
  if (startsWithBytes(buf, UTF16LE_BOM)) return { buf: buf.slice(UTF16LE_BOM.length), bom: "utf16le" };
  if (startsWithBytes(buf, UTF16BE_BOM)) return { buf: buf.slice(UTF16BE_BOM.length), bom: "utf16be" };
  return { buf, bom: null };
}
