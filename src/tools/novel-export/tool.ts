...
import type { NovelChapterOrder, NovelExportArgs, NovelExportFormat, NovelExportResultJson } from "./types"
import { markdownToHtml, wrapHtmlDocument } from "./render"

function sortByTimelineKey(a: { date?: string; start?: string; chapter_id: string }, b: { date?: string; start?: string; chapter_id: string }): number {
...
export function createNovelExportTool(deps: { projectRoot: string; config: NovelConfig }): ToolDefinition {
  return tool({
    description: "Export novel by merging chapters into a single MD/HTML file (deterministic ordering).",
    args: {
    ...
    async execute(args: NovelExportArgs) {
      ...
      if (chapterOrder === "by_id") {
        ...
        const rest = ordered.filter((c) => !pickedSet.has(c.chapter_id)).sort((a, b) => a.chapter_id.localeCompare(b.chapter_id))
        ordered = [...picked, ...rest]
      }

      const chapterInfos = ordered.map((c) => ({ chapter_id: c.chapter_id, title: c.title, path: c.path }))
      ...
      for (const chapter of ordered) {
        ...
        const titleForHeading = chapter.title ?? (typeof parsed.data.title === "string" ? parsed.data.title : chapter.chapter_id)
      ...
      if (writeFile) {
        if (format === "md") {
          ...
          const html = wrapHtmlDocument({ title, bodyHtml: htmlBody, language: deps.config.language })
