...
function parseExplicitRelations(
  ...
  for (const entity of entityPaths) {
    ...
    const parsed = parseFrontmatter<Record<string, unknown>>(content, { file: entity.path, strict: false })
...
export function createNovelGraphTool(deps: { projectRoot: string; config: NovelConfig }): ToolDefinition {
  return tool({
    description:
    ...
    async execute(args: NovelGraphArgs) {
      ...
      if (kind === "relationships") {
        ...
        if (explicit.length > 0) {
          ...
          nodes = Array.from(nodeIds).sort((a, b) => a.localeCompare(b)).map((id) => `${id}["${id}"]`)
          edges = explicit
            .sort((a, b) =>
