...
function discoverCommandsFromDir(dir: string, scope: CommandScope): CommandInfo[] {
  ...
  for (const entry of entries) {
    ...
    const parsed = parseFrontmatter<Record<string, unknown>>(content, { file: fullPath, strict: false })
    const description = typeof parsed.data.description === "string" ? parsed.data.description : undefined
    const argumentHint = typeof parsed.data["argument-hint"] === "string" ? (parsed.data["argument-hint"] as string) : undefined
...
export function discoverAllCommands(options: { projectRoot: string; disabledCommands?: string[] }): CommandInfo[] {
