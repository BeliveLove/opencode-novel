...
import type { NovelEntityGapsArgs, NovelEntityGapsResultJson, MissingEntityRef, NovelEntityKind, OrphanEntity } from "./types"
...
export function createNovelEntityGapsTool(deps: { projectRoot: string; config: NovelConfig }): ToolDefinition {
  return tool({
    description:
    ...
    async execute(args: NovelEntityGapsArgs) {
      ...
      const referencesByKind: Record<NovelEntityKind, Map<string, Array<{ chapter_id: string; path: string }>>> = {
        characters: new Map(),
        threads: new Map(),
        factions: new Map(),
        locations: new Map(),
      ...
      for (const kind of kinds) {
        ...
        for (const id of referencedIds) {
          ...
          const referencedBy = (referencesByKind[kind].get(id) ?? [])
            .sort((a, b) => a.chapter_id.localeCompare(b.chapter_id) || a.path.localeCompare(b.path))
          missing.push({
            kind,
            id,
            referencedBy,
      ...
      if (writeReport) {
        writeTextFile(reportPathAbs, renderEntityGapsMd({ missing, orphans }), { mode: "if-changed" })
      }

      // Cache (optional but recommended)
      const cachePathAbs = path.join(rootDir, deps.config.index.cacheDir, "entity-gaps.json")
      writeTextFile(cachePathAbs, JSON.stringify({ version: 1, missing, orphans }, null, 2) + "\n", {
        mode: "always",
      })
