...
export function createSkillTool(options: { projectRoot: string; disabledSkills?: string[] }): ToolDefinition {
  let cached: LoadedSkill[] | null = null

  const getSkills = (): LoadedSkill[] => {
    if (cached) return cached
    cached = discoverAllSkills({ projectRoot: options.projectRoot, disabledSkills: options.disabledSkills })
    return cached
  }
  ...
  return tool({
    ...
    async execute(args: { name: string }, ctx?: { agent?: string }) {
      ...
      return [
        `## Skill: ${skill.name}`,
        "",
        skill.definition.template.trim(),
        "",
      ].join("\n")
    },
  })
