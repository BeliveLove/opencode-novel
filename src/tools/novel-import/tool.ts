...
import type { ImportMapItem, NovelImportArgs, NovelImportMode, NovelImportResultJson } from "./types"
...
function detectChaptersFromContent(options: {
  ...
  for (let i = 0; i < options.lines.length; i += 1) {
    ...
    if (strongMatched && strongPattern) {
      ...
      hits.push({ lineIndex: i, headingLine: line, title, numeric: numeric ?? undefined, warnings })
      continue
    }
    ...
    if (sawStrong && options.enableLooseAfterStrong) {
      ...
      if (match) {
        ...
        hits.push({ lineIndex: i, headingLine: line, title, warnings: ["使用松匹配 H1 作为章节分割"] })
      }
    }
  }
...
function buildChapterId(options: {
  ...
  if (typeof options.numeric === "number" && Number.isFinite(options.numeric) && options.numeric > 0) {
...
function renderImportReport(options: {
  ...
  for (const item of options.imported) {
    lines.push(`| ${item.chapter_id} | ${item.title ?? ""} | ${item.source_path} | ${item.output_path} |`)
  }
  lines.push("")
...
export function createNovelImportTool(deps: { projectRoot: string; config: NovelConfig }): ToolDefinition {
  return tool({
    description:
    ...
    async execute(args: NovelImportArgs) {
      ...
      const enableLooseAfterStrong = deps.config.import.chapterDetection.enableLooseH1AfterFirstMatch
      ...
      importMap.sort((a, b) => a.chapter_id.localeCompare(b.chapter_id) || a.output_path.localeCompare(b.output_path))
