...
type RuleContext = {
  ...
  chaptersById: Array<{ chapter_id: string; path: string; timeline?: { date?: string; start?: string; end?: string; location?: string }; characters?: string[]; threads_opened?: string[]; threads_advanced?: string[]; threads_closed?: string[]; pov?: string }>
  strictMode: boolean
...
function parseChapterInterval(chapter: { timeline?: { date?: string; start?: string; end?: string } }): { startMs?: number; endMs?: number; errors: string[] } {
  const errors: string[] = []
...
const RULES: Rule[] = [
  {
    ...
    run(ctx) {
      ...
      for (const chapter of ctx.chaptersById) {
        ...
        if (interval.startMs !== undefined && interval.endMs !== undefined && interval.startMs > interval.endMs) {
          findings.push({
  ...
  },
  {
    ...
    run(ctx) {
      ...
      for (const thread of ctx.scan.entities.threads) {
        ...
        const parsed = parseFrontmatter<Record<string, unknown>>(content, { file: thread.path, strict: false })
        ...
        for (const match of body.matchAll(/\b(R-[0-9]{3,}|R-[A-Z]+-[0-9]{1,})\b/g)) {
          ...
          if (!ctx.bibleRuleIds.has(id)) {
            findings.push({
              ...
              evidence: [ctx.evidence(chapter.path, body.slice(Math.max(0, match.index ?? 0), Math.min(body.length, (match.index ?? 0) + 80)))],
...
export function createNovelContinuityCheckTool(deps: { projectRoot: string; config: NovelConfig }): ToolDefinition {
  return tool({
    description: "Run continuity rules and produce CONTINUITY_REPORT.md (deterministic findings ordering).",
    args: {
    ...
    async execute(args: NovelContinuityArgs) {
      ...
      let chaptersById = [...scan.entities.chapters].sort((a, b) => a.chapter_id.localeCompare(b.chapter_id))
      ...
      const ctx: RuleContext = {
        ...
        chaptersById,
        strictMode,
        evidence: (file, excerpt) => ({ file, excerpt: excerpt ? excerpt.slice(0, 200) : undefined }),
        chapterIdOrder,
        bibleRuleIds,
      }
      ...
      if (deps.config.continuity.enabled !== false) {
        for (const rule of RULES) {
          ...
          try {
            const f = rule.run(ctx).map((x) => ({ ...x, severity: x.severity ?? rule.defaultSeverity }))
            findingsRaw.push(...f)
          } catch (error) {
      ...
      if ((!args.scope || args.scope.kind === "all") && deps.config.continuity.enabled !== false) {
        ...
        const intervalsByChar = new Map<string, Array<{ startMs: number; endMs: number; chapter_id: string; path: string; location?: string }>>()
        ...
        for (const [charId, list] of intervalsByChar.entries()) {
          const sorted = [...list].sort((a, b) => a.startMs - b.startMs || a.chapter_id.localeCompare(b.chapter_id))
        ...
        findings.push(...applyRuleConfig({ config: deps.config, strictMode, findings: overlapFindings }))
      }
      ...
      if (writeReport) {
        writeTextFile(reportPathAbs, renderContinuityReportMd({ stats: { errors, warns, infos }, findings }), { mode: "if-changed" })
      }
