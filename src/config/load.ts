...
function parseJsoncFile(path: string): { data: PlainObject | null; errors: string[] } {
  ...
  if (parseErrors.length > 0) {
    errors.push(
      ...parseErrors.map((e) => `JSONC 解析失败: code=${e.error} offset=${e.offset} length=${e.length}`),
    )
  }
...
export function loadNovelConfig(projectRoot: string): {
  ...
  const merged = deepMerge(deepMerge(defaultConfig, userConfig ?? undefined), projectConfig ?? undefined) as PlainObject
